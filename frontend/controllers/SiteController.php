...
class SiteController extends Controller
{
    ...
    public function actionSignup()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->signup()) {
            Yii::$app->session->setFlash('success', 'Спасибо за регистрацию! Проверьте вашу почту для подтверждения регистрации');
            return $this->goHome();
        }
    ...
    public function actionRequestPasswordReset()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                Yii::$app->session->setFlash('success', 'Проверьте вашу почту для дальнейших действий');
    ...
    public function actionVerifyEmail($token)
    {
        ...
        if (($user = $model->verifyEmail()) && Yii::$app->user->login($user)) {
            Yii::$app->session->setFlash('success', 'Ваша почта была изменена');
            return $this->goHome();
        }
